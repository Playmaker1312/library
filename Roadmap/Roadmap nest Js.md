...
2.2  Tipe Data Dasar (string, number, boolean, any, unknown, void, never)
...
5.4  Instalasi NestJS CLI (`npm i -g @nestjs/cli`)
5.5  Membuat project pertama (`nest new project-name`)
5.6  Struktur folder & file project NestJS
5.7  Menjalankan server development (`npm run start:dev`)
...
6.3  Membuat Module (`nest g module <name>`)
...
7.2  Membuat Controller (`nest g controller <name>`)
...
8.3  Membuat Service (`nest g service <name>`)
...
|Sumber|Keterangan|
|---|---|
|**docs.nestjs.com**|Dokumentasi resmi (UTAMA)|
